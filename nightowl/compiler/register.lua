...
return function(Compiler)
    ...
    function Compiler:allocRegister(isVar)
        ...
        if isVar then self.registers[id] = self.VAR_REGISTER
        else self.registers[id] = true end
        return id
    end
    ...
    function Compiler:register(scope, id)
        ...
        if id < MAX_REGS then
            ...
            return self.Ast.VariableExpression(self.containerFuncScope, vid)
        end
        local vid = self:getRegisterVarId(MAX_REGS)
        scope:addReferenceToHigherScope(self.containerFuncScope, vid)
        return self.Ast.IndexExpression(
            self.Ast.VariableExpression(self.containerFuncScope, vid),
            self.Ast.NumberExpression((id - MAX_REGS) + 1)
        )
    end
    ...
    function Compiler:registerAssignment(scope, id)
        ...
        if id < MAX_REGS then
            local vid = self:getRegisterVarId(id)
            scope:addReferenceToHigherScope(self.containerFuncScope, vid)
            return self.Ast.AssignmentVariable(self.containerFuncScope, vid)
        end
        local vid = self:getRegisterVarId(MAX_REGS)
        scope:addReferenceToHigherScope(self.containerFuncScope, vid)
        return self.Ast.AssignmentIndexing(
            self.Ast.VariableExpression(self.containerFuncScope, vid),
            self.Ast.NumberExpression((id - MAX_REGS) + 1)
        )
    end
    ...
    function Compiler:setRegister(scope, id, val, compoundArg)
        ...
        return self.Ast.AssignmentStatement({ self:registerAssignment(scope, id) }, { val })
    end

    function Compiler:setRegisters(scope, ids, vals)
        local lhs = {}
        for _, id in ipairs(ids) do table.insert(lhs, self:registerAssignment(scope, id)) end
        return self.Ast.AssignmentStatement(lhs, vals)
    end
    ...
    function Compiler:copyRegisters(scope, to, from)
        ...
        if #lhs > 0 then return self.Ast.AssignmentStatement(lhs, vals) end
    end
    ...
    function Compiler:pos(scope)
        ...
        return self.Ast.VariableExpression(self.containerFuncScope, self.posVar)
    end

    function Compiler:posAssignment(scope)
        scope:addReferenceToHigherScope(self.containerFuncScope, self.posVar)
        return self.Ast.AssignmentVariable(self.containerFuncScope, self.posVar)
    end

    function Compiler:args(scope)
        scope:addReferenceToHigherScope(self.containerFuncScope, self.argsVar)
        return self.Ast.VariableExpression(self.containerFuncScope, self.argsVar)
    end

    function Compiler:unpack(scope)
        scope:addReferenceToHigherScope(self.scope, self.unpackVar)
        return self.Ast.VariableExpression(self.scope, self.unpackVar)
    end

    function Compiler:env(scope)
        scope:addReferenceToHigherScope(self.scope, self.envVar)
        return self.Ast.VariableExpression(self.scope, self.envVar)
    end

    function Compiler:jmp(scope, to)
        scope:addReferenceToHigherScope(self.containerFuncScope, self.posVar)
        return self.Ast.AssignmentStatement(
            { self.Ast.AssignmentVariable(self.containerFuncScope, self.posVar) },
            { to }
        )
    end
    ...
    function Compiler:setPos(scope, val)
        if not val then
            local randKey = self.Ast.StringExpression("__owl_" .. tostring(math.random(100000, 999999)))
            scope:addReferenceToHigherScope(self.containerFuncScope, self.posVar)
            return self.Ast.AssignmentStatement(
                { self.Ast.AssignmentVariable(self.containerFuncScope, self.posVar) },
                { self.Ast.IndexExpression(self:env(scope), randKey) }
            )
        end
        scope:addReferenceToHigherScope(self.containerFuncScope, self.posVar)
        return self.Ast.AssignmentStatement(
            { self.Ast.AssignmentVariable(self.containerFuncScope, self.posVar) },
            { self.Ast.NumberExpression(val) }
        )
    end
    ...
    function Compiler:setReturn(scope, val)
        ...
        return self.Ast.AssignmentStatement(
            { self.Ast.AssignmentVariable(self.containerFuncScope, self.returnVar) },
            { val }
        )
    end
    ...
    function Compiler:getReturn(scope)
        scope:addReferenceToHigherScope(self.containerFuncScope, self.returnVar)
        return self.Ast.VariableExpression(self.containerFuncScope, self.returnVar)
    end

    function Compiler:returnAssignment(scope)
        scope:addReferenceToHigherScope(self.containerFuncScope, self.returnVar)
        return self.Ast.AssignmentVariable(self.containerFuncScope, self.returnVar)
    end
    ...
    function Compiler:setUpvalueMember(scope, idExpr, valExpr, compoundConstructor)
        ...
        if compoundConstructor then
            return compoundConstructor(
                self.Ast.AssignmentIndexing(self.Ast.VariableExpression(self.scope, self.upvaluesTable), idExpr),
                valExpr
            )
        end
        return self.Ast.AssignmentStatement(
    ...
    function Compiler:getUpvalueMember(scope, idExpr)
        scope:addReferenceToHigherScope(self.scope, self.upvaluesTable)
        return self.Ast.IndexExpression(
            self.Ast.VariableExpression(self.scope, self.upvaluesTable),
            idExpr
        )
    end
end
