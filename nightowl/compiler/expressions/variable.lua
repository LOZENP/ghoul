        ...
        if i == 1 then
            ...
            if expression.scope.isGlobal then
                ...
                self:addStatement(self:setRegister(scope, tmpReg,
                    ...
                    Ast.StringExpression(expression.scope:getVariableName(expression.id))), {tmpReg}, {}, false)
 
                self:addStatement(self:setRegister(scope, regs[i],
 
                    Ast.IndexExpression(self:env(scope), self:register(scope, tmpReg))), {regs[i]}, {tmpReg}, true)
                ...
                if self.scopeFunctionDepths[expression.scope] == funcDepth then
                    ...
                    if self:isUpvalue(expression.scope, expression.id) then
                        ...
                        self:addStatement(self:setRegister(scope, reg,
                            ...
                            self:getUpvalueMember(scope, self:register(scope, varReg))), {reg}, {varReg}, true)
