...
return function(self, expression, funcDepth, numReturns)
    ...
    if expression.rhs.isConstant then
        ...
        self:addStatement(self:setRegister(scope, resReg,
    ...
    local block1 = self:createBlock()
 
    local block2 = self:createBlock()
    ...
    do
        ...
        local sc = block1.scope
 
        local rhsReg = self:compileExpression(expression.rhs, funcDepth, 1)[1]
 
        self:addStatement(self:copyRegisters(sc, {resReg}, {rhsReg}), {resReg}, {rhsReg}, false)
 
        self:freeRegister(rhsReg, false)
 
        self:addStatement(self:setRegister(sc, self.POS_REGISTER,
 
            Ast.NumberExpression(block2.id)), {self.POS_REGISTER}, {}, false)
