...
return function(self, statement, funcDepth)
    ...
    if #statement.indices > 0 then
        ...
        if statement.scope.isGlobal then
            ...
            self:addStatement(self:setRegister(scope, tblReg,
 
                Ast.StringExpression(statement.scope:getVariableName(statement.id))), {tblReg}, {}, false)
 
            self:addStatement(self:setRegister(scope, tblReg,
 
                Ast.IndexExpression(self:env(scope), self:register(scope, tblReg))), {tblReg}, {tblReg}, true)
            ...
            if self.scopeFunctionDepths[statement.scope] == funcDepth then
                ...
                if self:isUpvalue(statement.scope, statement.id) then
                    ...
                    self:addStatement(self:setRegister(scope, tblReg,
                        ...
                        self:getUpvalueMember(scope, self:register(scope, reg))), {tblReg}, {reg}, true)
        ...
        for i = 1, #statement.indices - 1 do
            ...
            local idx = statement.indices[i]
 
            local indexReg = self:compileExpression(Ast.StringExpression(idx), funcDepth, 1)[1]
            ...
            self:addStatement(self:setRegister(scope, tblReg,
                ...
                Ast.IndexExpression(self:register(scope, old), self:register(scope, indexReg))),
 
                {tblReg}, {tblReg, indexReg}, false)
 
            self:freeRegister(old,      false)
        ...
        local idx = statement.indices[#statement.indices]
    ...
    if statement.scope.isGlobal then
        ...
        self:addStatement(self:setRegister(scope, tmpReg,
            ...
            Ast.StringExpression(statement.scope:getVariableName(statement.id))), {tmpReg}, {}, false)
        ...
        if self.scopeFunctionDepths[statement.scope] == funcDepth then
            ...
            if self:isUpvalue(statement.scope, statement.id) then
                ...
                self:addStatement(self:setUpvalueMember(scope,
                    ...
                    self:register(scope, reg), self:register(scope, retReg)),
